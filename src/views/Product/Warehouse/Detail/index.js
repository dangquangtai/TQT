...
import { createProductWarehouse, updateProductWarehouse } from './../../../../services/api/Product/Warehouse';
...
const ProductWarehouseModal = () => {
  ...
  const buttonSave = formButtons.find((button) => button.name === view.productWarehouse.detail.save);
  const { productWarehouseDocument: openDialog } = useSelector((state) => state.floatingMenu);
  ...
  const handleCloseDialog = () => {
    ...
    dispatch({ type: FLOATING_MENU_CHANGE, productWarehouseDocument: false });
  };
  ...
  const setDocumentToDefault = async () => {
    setProductWarehouseData({ address: '', is_active: true });
    setTabIndex(0);
  };

  const handleChanges = (e) => {
    const { name, value } = e.target;
    setProductWarehouseData({ ...productWarehouseData, [name]: value });
  };

  const handleSubmit = async () => {
    try {
      if (selectedDocument) {
        await updateProductWarehouse(productWarehouseData);
        handleOpenSnackbar('success', 'Cập nhật kho thành phẩm thành công!');
      } else {
                      ...
                      <div className={classes.tabItem}>
                        ...
                        <div className={classes.tabItemBody}>
                          <Grid container className={classes.gridItem} alignItems="center">
                            <Grid item lg={4} md={4} xs={4}>
                              <span className={classes.tabItemLabelField}>
                                Tên kho<sup className="required-star">*</sup>
                              </span>
                            </Grid>
                          ...
                          <Grid container className={classes.gridItem} alignItems="center">
                            ...
                            <Grid item lg={8} md={8} xs={8}>
                              <TextField
                                ...
                                variant="outlined"
                                name="province_id"
                                value={productWarehouseData?.province_id || ''}
                                size="small"
                                onChange={handleChanges}
                              >
                          ...
                          <Grid container className={classes.gridItemInfo} alignItems="center">
                            ...
                            <Grid item lg={8} md={8} xs={8}>
                              <Switch
                                checked={productWarehouseData.is_active || true}
                                onChange={(e) => setProductWarehouseData({ ...productWarehouseData, is_active: e.target.checked })}
                                color="primary"
                              />
                            </Grid>
                          </Grid>
