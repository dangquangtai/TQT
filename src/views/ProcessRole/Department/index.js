...
const ProcessRoleDeptModal = () => {
  ...
  const handleUpdateRole = async () => {
    try {
      let dept = [];
      let data;
      deptSelected.map((row, index) => ((data = row.department_code + '_' + roletemplate[index].id), dept.push(data)));
                            ...
                            <Table sx={{ minWidth: 650 }} aria-label="simple table">
                              ...
                              <TableBody>
                                {deptSelected?.map((row, index) => (
                                  <TableRow key={row.department_name + '_'} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
