...
import { Tooltip, Button, Table, TableBody, TableHead, TableRow, TableCell, TableContainer } from '@material-ui/core';
import RemoveCircleOutlineIcon from '@material-ui/icons/RemoveCircleOutline';
import { useStyles } from './../../Table/style';
import useDepartment from '../../../hooks/useDepartment';

const ProcessRoleDeptModal = (props) => {
  const { process_role_code_selected, handleRemoveDept, buttonRemoveDeptRole, changeDeptReload } = props;
          ...
          <TableHead>
            <TableRow>
              ...
              <TableCell align="right">
                <span style={{ minWidth: 100, maxWidth: 100 }}></span>
              </TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {DeptList?.map((row, index) => {
              return (
                <TableRow className={classes.tableRow} hover tabIndex={-1} key={row.key}>
                  ...
                  <TableCell align="left">
                    <>
                      <span className={classes.tableItemID}>
                  ...
                  <TableCell>
                    {buttonRemoveDeptRole && (
                      <Tooltip title={buttonRemoveDeptRole.text}>
                        <Button className={`${classes.handleButton} ${classes.handleButtonNote}`} onClick={() => handleRemoveDept(row.id)}>
